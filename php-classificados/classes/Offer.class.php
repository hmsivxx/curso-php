<?php
    ...
    class Offer
    {
        public function getTotalOffers($filters)
        {
            ...
            $sql = $conn->prepare("SELECT COUNT(*) as count FROM offers WHERE ".implode(' AND ', $filterString));
            if (!empty($filters['category'])) {
        ...
        public function addNewOffer($title, $category, $price, $description, $condition)
        {
            ...
            $sql = $conn->prepare("INSERT INTO offers SET id_user = :id_user, title = :title, id_category = :id_category, price = :price, description = :description, prod_cond = :condition");
        ...
        public function deleteOffer($id)
        {
            ...
            $sql = $conn->prepare("DELETE FROM images_offers WHERE id_offer = :id_offer");
            $sql->bindValue(":id_offer", $id);
            $sql->execute();

            $sql = $conn->prepare("DELETE FROM offers WHERE id = :id");
            $sql->bindValue(":id", $id);
            $sql->execute();
        }
        ...
        public function editOffer($title, $category, $price, $description, $condition, $images, $id)
        {
            ...
            $sql = $conn->prepare("UPDATE offers SET id_user = :id_user, title = :title, id_category = :id_category, price = :price, description = :description, prod_cond = :condition WHERE id = :id");
            $sql->bindValue(":title", $title);
            $sql->bindValue(":id_category", (int)$category);
            $sql->bindValue(":id_user", (int)$_SESSION['clogin']);
            $sql->bindValue(":price", (float)$price);
            ...
            if (count($images) > 0) {
                ...
                for ($i = 0; $i < $imgCount; $i++) {
                    ...
                    if (in_array($type, ['image/jpeg', 'image/png'])) {
                        ...
                        $sql = $conn->prepare("INSERT INTO images_offers SET id_offer = :id_offer, url = :url");
                        $sql->bindValue(":id_offer", $id);
        ...
        public function getOffer($id)
        {
            ...
            if ($sql->rowCount() > 0) {
                ...
                $sql = $conn->prepare("SELECT id, url FROM images_offers WHERE id_offer = :id_offer");
                $sql->bindValue(":id_offer", $id);
                $sql->execute();
